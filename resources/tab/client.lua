    ...
    function()
        ...
        while true do
            if (tabEnabled) then
                ...
                loadAnimDict( "amb@world_human_seat_wall_tablet@male@exit" )
                Citizen.Wait(1)
                TaskPlayAnim(ped, "amb@world_human_seat_wall_tablet@male@exit", "exit", 8.0, 2.0, -1, 50, 2.0, 0, 0, 0 )
...
RegisterCommand("mdt", function(source, args, rawCommand)
    ...
    --ClearPedTasksImmediately(ped)
    --TaskStartScenarioInPlace(ped, "WORLD_HUMAN_SEAT_WALL_TABLET", 0, false)
    Citizen.Wait(0)
end, false)
